...
export default function EnterCodeModal(props) {
    ...
    const approveRequestSubmitHandler = (e) => {
        e.preventDefault();
        const fetch = async () => {
            let code = formRef?.current?.points?.value || 0;
            const res = await userService.approveRequestByID(props?.codeRes?.id, code);
            if (res.length === 0) {
                toast("Invalid code");
                return
            }
            ...
            <div className="absolute w-[100vw] ">
                ...
                <form ref={formRef} onSubmit={approveRequestSubmitHandler} className="xsm:mt-[-5%] xsm:w-[85vw] w-[43.92386530014641vw] m-auto flex flex-col items-start mt-[30px] relative z-20 bg-white rounded-[0.3rem]">
                    ...
                    <div className="flex flex-col gap-[15px] px-[15px] ">
                        ...
                        <div className="flex gap-[8px]">
                            <label className="font-medium text-[18px]">Remaining Points: </label><h2>{props?.codeRes?.offerType === "punch" ? props?.codeRes?.pointsStatus : props?.codeRes?.offerPrice}</h2>
                        </div>
                        {props?.codeRes?.offerType !== "free" &&
                            <div className="flex gap-[8px]">
